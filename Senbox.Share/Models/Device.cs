...
namespace Senbox.Share.Models
{
    public class Device : IDevice, IDisposable
    {
        ...
        private void InitMqtt()
        {
            ...
            _mqtt.Connect(clientId, mqttUserName, _clientKey, true, 1);
        }
        ...
        public string GetValue(object asset)
        {
            string resultContent = null;
            try
            {
                string uri = "/device/" + DeviceId + "/asset/" + asset.ToString() + "/state";
                HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, uri); // Création de la requête
                PrepareRequestForAuth(request);
                var task = _http.SendAsync(request, HttpCompletionOption.ResponseContentRead); // Exécution

                using (var result = task.Result)
                {
                    using (HttpContent content = result.Content)
                    {
                        var contentTask = content.ReadAsStringAsync(); // On récupère le résultat dans contentTask
                        resultContent = contentTask.Result; // Et on stocke ce qui nous intéresse dans le string
                        result.EnsureSuccessStatusCode();
                    }
                }
            }
